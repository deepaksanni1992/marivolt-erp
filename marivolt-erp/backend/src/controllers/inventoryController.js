...
export async function getBalance(req, res) {
  try {
    ...
    const row = await StockBalance.findOne({ itemCode, warehouse }).lean();
    if (!row) {
      return res.json({
        itemCode,
        warehouse,
...
export async function postStockIn(req, res) {
  try {
    ...
    const bal = await StockBalance.findOne({ itemCode: code, warehouse: w }).lean();
...
export async function postOpening(req, res) {
  try {
    ...
    const existing = await StockBalance.findOne({ itemCode: code, warehouse: w });
