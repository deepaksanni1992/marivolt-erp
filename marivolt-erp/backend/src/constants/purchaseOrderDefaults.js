/** Marivolt FZE standard purchase order wording (matches issued PDF format). */

export const DEFAULT_BUYER = {
  buyerLegalName: "Marivolt FZE",
  buyerAddressLine: "LV09B, Hamriyah freezone phase 2, Sharjah, UAE",
  buyerPhone: "+971-543053047",
  buyerEmail: "sales@marivolt.co",
  buyerWeb: "www.marivolt.co",
};
...
export function applyPurchaseOrderDefaults(body) {
  const b = { ...body };
  for (const [k, v] of Object.entries(DEFAULT_BUYER)) {
    if (b[k] === undefined || b[k] === null || b[k] === "") b[k] = v;
  }
  for (const [k, v] of Object.entries(DEFAULT_COMMERCIAL)) {
    if (b[k] === undefined || b[k] === null || b[k] === "") b[k] = v;
  }
  if (b.specialRemarks === undefined || b.specialRemarks === null || b.specialRemarks === "") {
    b.specialRemarks = DEFAULT_SPECIAL_REMARKS;
  }
  if (b.closingNote === undefined || b.closingNote === null || b.closingNote === "") {
    b.closingNote = DEFAULT_CLOSING_NOTE;
  }
  if (!b.termsAndConditions || !String(b.termsAndConditions).trim()) {
    b.termsAndConditions = DEFAULT_PURCHASE_TERMS;
  }
  return b;
}
