    ...
    <div className="space-y-3">
      ...
      <div className="grid gap-3 lg:grid-cols-[1.4fr_1fr]">
        ...
        <div className="rounded-2xl border bg-white p-3">
          ...
          {selected ? <div className="mt-2 space-y-2 text-sm"><div><b>{selected.grnNo}</b></div><div>PO: {selected.poNo || "—"}</div><div>Supplier: {selected.supplierName || "—"}</div><div>Status: <StatusBadge status={selected.status} /></div><div>Approval: <StatusBadge status={selected.approvalStatus} /></div><div>Lines: {(detail?.items || selected.items || []).length}</div>{String((detail || selected).sourceType || "").toUpperCase() === "ASN_RECEIVING" ? <div className="rounded-lg border border-sky-200 bg-sky-50 p-2 text-xs text-sky-950"><div className="font-semibold">{String((detail || selected).status || "").toUpperCase() === "DRAFT" ? "Draft GRN — ASN Receiving" : "Posted GRN — ASN Receiving"}</div><p className="mt-1">Source: ASN → PO. POST GRN receives accepted qty only. RU labels are not reprinted.</p></div> : null}<div className="flex flex-wrap gap-2 pt-2"><button className="rounded border px-2 py-1 text-xs" type="button" onClick={() => setEditOpen(true)}>Edit Lines</button>{String((detail || selected).sourceType || "").toUpperCase() === "ASN_RECEIVING" ? (String((detail || selected).status || "").toUpperCase() === "DRAFT" ? <button className="rounded border px-2 py-1 text-xs" type="button" onClick={() => { if (window.confirm(`POST GRN ${selected.grnNo}?`)) act.mutate({ grnNo: selected.grnNo, action: "post" }); }}>POST GRN</button> : null) : <button className="rounded border px-2 py-1 text-xs" type="button" onClick={() => act.mutate({ grnNo: selected.grnNo, action: "receive" })}>Receive</button>}<button className="rounded border px-2 py-1 text-xs" type="button" onClick={() => act.mutate({ grnNo: selected.grnNo, action: "cancel" })}>Cancel</button><button className="rounded border px-2 py-1 text-xs" type="button" onClick={() => act.mutate({ grnNo: selected.grnNo, action: "close" })}>Close</button></div></div> : <div className="mt-2 text-sm text-slate-500">Select a GRN row.</div>}
        </div>
      </div>
      <div className="rounded-2xl border bg-white p-3">
