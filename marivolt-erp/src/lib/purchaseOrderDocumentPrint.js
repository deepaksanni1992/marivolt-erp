...
export function buildPurchaseOrderDocumentHtml(doc, company = null) {
  ...
  const docSubtitle = b.useBrandedLayout
    ? escapeHtml(b.companySubtitle || "Buyer reference document — quotation-style layout")
  ...
  const lineRows = lines
    .map((l, i) => {
      ...
      return `<tr style="background:${stripe}">
        ...
        <td style="border:1px solid #e5e7eb;padding:6px 8px;font-family:ui-monospace,monospace;font-size:11px">${escapeHtml(l.articleNo || l.itemCode || "—")}</td>
        <td style="border:1px solid #e5e7eb;padding:6px 8px;font-size:12px">${escapeHtml(l.description || "—")}</td>
        <td style="border:1px solid #e5e7eb;padding:6px 8px;font-family:ui-monospace,monospace;font-size:11px">${escapeHtml(l.partNo || "—")}</td>
  ...
  const headerDefault = `
    <header style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;border-bottom:2px solid #e5e7eb;padding-bottom:20px;margin-bottom:20px;align-items:start" class="po-print-header">
      <div style="display:flex;gap:12px;align-items:flex-start">
        <div style="width:120px;height:100px;display:flex;align-items:center;justify-content:center;flex-shrink:0">
          ${logoSrc ? `<img src="${escapeHtml(logoSrc)}" alt="" style="max-height:100px;max-width:208px;object-fit:contain"/>` : `<div style="font-size:22px;font-weight:800;color:#1f5a96">${escapeHtml(String(company?.code || "MV").slice(0, 4))}</div>`}
        </div>
        <div>
          <div style="font-size:11px;font-weight:700;letter-spacing:0.18em;color:#6b7280;text-transform:uppercase">Purchase order</div>
          <div style="margin-top:6px;font-family:ui-monospace,monospace;font-size:18px;font-weight:700;color:#111">${poNo}</div>
          <div style="margin-top:8px;font-size:13px;color:#555">
            <div><span style="font-weight:600;color:#6b7280">Date: </span>${formatPoDate(doc.orderDate)}</div>
            <div><span style="font-weight:600;color:#6b7280">Currency: </span>${cur}</div>
            ${doc.ref ? `<div><span style="font-weight:600;color:#6b7280">Ref: </span>${escapeHtml(doc.ref)}</div>` : ""}
          </div>
        </div>
      </div>
      <div style="text-align:center;padding-top:4px">
        <div style="font-size:26px;font-weight:700;color:#1f3a5f">PURCHASE ORDER</div>
        <div style="margin-top:6px;font-size:13px;font-weight:600;color:#2c5282">${docSubtitle}</div>
        <div style="margin-top:12px">
          ${unsaved ? `<span style="padding:4px 12px;font-size:11px;font-weight:600;background:#f3f4f6;border-radius:9999px;border:1px solid #e5e7eb">Unsaved draft</span>` : doc.status ? `<span style="padding:4px 12px;font-size:11px;font-weight:600;text-transform:uppercase;background:#f3f4f6;border-radius:9999px;border:1px solid #e5e7eb">${escapeHtml(doc.status)}</span>` : ""}
        </div>
      </div>
      <div style="text-align:right;font-size:12px;color:#4a5568">
        <div style="font-size:28px;font-weight:800;line-height:1.2;color:#1f3a5f">${buyer.name !== "—" ? buyer.name : escapeHtml(b.companyDisplayName || company?.name || "—")}</div>
      </div>
    </header>`;
...
<body>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
    <div style="border:1px solid #e5e7eb;border-radius:10px;background:#fafafa;padding:12px;min-height:120px">
  ...
  <table style="margin-top:12px">
    <thead>
      <tr style="background:${thBg};color:${thColor}">
        <th style="border:1px solid ${thBorder};padding:8px;text-align:left;font-size:11px;text-transform:uppercase">Pos</th>
        <th style="border:1px solid ${thBorder};padding:8px;text-align:left;font-size:11px;text-transform:uppercase">Article Nr.</th>
        <th style="border:1px solid ${thBorder};padding:8px;text-align:left;font-size:11px;text-transform:uppercase">Description</th>
        <th style="border:1px solid ${thBorder};padding:8px;text-align:left;font-size:11px;text-transform:uppercase">Part Nr.</th>
        ...
        <th style="border:1px solid ${thBorder};padding:8px;text-align:left;font-size:11px;text-transform:uppercase">Remark</th>
      </tr>
    </thead>
